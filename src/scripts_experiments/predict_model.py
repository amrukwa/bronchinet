...
from common.constant import (
    BASEDIR,
    NAME_TESTINGDATA_RELPATH,
    SIZE_IN_IMAGES,
    IS_GENERATE_PATCHES,
    PROP_OVERLAP_SLIDE_WINDOW_TEST,
    TYPE_LOSS,
    LIST_TYPE_METRICS,
    IS_VALID_CONVOLUTIONS,
    IS_MASK_REGION_INTEREST,
    NAME_TEMPO_POSTERIORS_RELPATH,
    NAME_REFERENCE_KEYS_PROCIMAGE_FILE,
    NAME_REFERENCE_KEYS_POSTERIORS_FILE,
    TYPE_DNNLIB_USED,
)
...
def main(args):
    ...
    in_reference_keys_file = workdir_manager.get_datafile_exist(
        args.name_input_reference_keys_file
    )
    output_predictions_path = workdir_manager.get_pathdir_new(
        args.name_output_predictions_relpath
    )
    out_reference_keys_file = workdir_manager.get_pathfile_new(
        args.name_output_reference_keys_file
    )
    ...
    if TYPE_DNNLIB_USED == "Keras":
        ...
        model_trainer.create_loss(
            type_loss=args.type_loss,
            is_mask_to_region_interest=args.is_mask_region_interest,
        )
        model_trainer.create_list_metrics(
            list_type_metrics=args.list_type_metrics,
            is_mask_to_region_interest=args.is_mask_region_interest,
        )
    ...
    if args.is_save_featmaps_layer:
        print(
            "Compute and store Feature Maps from the Model layer '%s'..."
            % (args.name_layer_save_featmaps)
        )
        network_checker = get_network_checker(
            args.size_in_images, model_trainer._network
        )
    else:
    ...
    for ifile, in_image_file in enumerate(list_test_images_files):
        ...
        print("Loading data...")
        image_data_loader = get_train_imagedataloader_1image(
            [in_image_file],
        ...
        print(
            "Loaded '%s' files. Total patches generated: '%s'..."
            % (1, len(image_data_loader))
        )
        ...
        if args.is_save_featmaps_layer:
            ...
            out_prediction_patches = network_checker.get_feature_maps(
                image_data_loader, args.name_layer_save_featmaps
            )
        else:
        ...
        if args.is_reconstruct_patches:
            ...
            out_prediction_reconstructed = image_reconstructor.compute_full(
                out_prediction_patches
            )
        else:
            out_prediction_reconstructed = np.squeeze(
                out_prediction_patches, axis=(0, -1)
            )
        ...
        if args.is_save_featmaps_layer:
            ...
            for ifeat in range(num_featmaps):
                output_prediction_file = join_path_names(
                    output_predictions_path,
                    name_output_prediction_files
                    % (in_caseproc_name, args.name_layer_save_featmaps, ifeat + 1),
                )
                print(
                    "Output: '%s', of dims '%s'..."
                    % (
                        basename(output_prediction_file),
                        out_prediction_reconstructed[..., ifeat].shape,
                    )
                )

                ImageFileReader.write_image(
                    output_prediction_file, out_prediction_reconstructed[..., ifeat]
                )

                outdict_reference_keys[basename_filenoext(output_prediction_file)] = (
                    basename(in_reference_key)
                )
            # endfor
        else:
            output_prediction_file = join_path_names(
            ...
            print(
                "Output: '%s', of dims '%s'..."
                % (basename(output_prediction_file), out_prediction_reconstructed.shape)
            )

            ImageFileReader.write_image(
                output_prediction_file, out_prediction_reconstructed
            )
    ...
    save_dictionary_csv(
        out_reference_keys_file.replace(".npy", ".csv"), outdict_reference_keys
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--is_reconstruct_patches", type=str2bool, default=IS_GENERATE_PATCHES
    )
    parser.add_argument(
        "--prop_overlap_slide_window",
        type=str2tuple_float,
        default=PROP_OVERLAP_SLIDE_WINDOW_TEST,
    )
    parser.add_argument("--type_loss", type=str, default=TYPE_LOSS)
    parser.add_argument(
        "--list_type_metrics", type=str2list_str, default=LIST_TYPE_METRICS
    )
    parser.add_argument(
        "--is_valid_convolutions", type=str2bool, default=IS_VALID_CONVOLUTIONS
    )
    parser.add_argument(
        "--is_mask_region_interest", type=str2bool, default=IS_MASK_REGION_INTEREST
    )
    parser.add_argument(
        "--name_output_predictions_relpath",
        type=str,
        default=NAME_TEMPO_POSTERIORS_RELPATH,
    )
    parser.add_argument(
        "--name_input_reference_keys_file",
        type=str,
        default=NAME_REFERENCE_KEYS_PROCIMAGE_FILE,
    )
    parser.add_argument(
        "--name_output_reference_keys_file",
        type=str,
        default=NAME_REFERENCE_KEYS_POSTERIORS_FILE,
    )
    ...
    if args.in_config_file:
        if not is_exist_file(args.in_config_file):
            ...
            print(
                "Set up experiments with parameters from file: '%s'"
                % (args.in_config_file)
            )

            # args.basedir = str(input_args_file['workdir'])
            args.size_in_images = str2tuple_int(input_args_file["size_in_images"])
            args.is_reconstruct_patches = str2bool(
                input_args_file["is_generate_patches"]
            )
            args.type_loss = str(input_args_file["type_loss"])
            args.list_type_metrics = str2list_str(input_args_file["list_type_metrics"])
            args.is_valid_convolutions = str2bool(
                input_args_file["is_valid_convolutions"]
            )
            args.is_mask_region_interest = str2bool(
                input_args_file["is_mask_region_interest"]
            )
    ...
    if args.is_filter_output_network:
        print(
            "Using the option to filter the network output to reduce border effects..."
        )
        args.size_filter_output_images = (
            NetworksUtil.calc_size_output_layer_valid_convols(args.size_in_images)
        )
        print(
            "Filtering the output images outside the window: %s..."
            % (str(args.size_filter_output_images))
        )
    else:
